...
# GitHub Copilot Instructions for saros.base

## Required R Development Commands

When working with this R package, **always** use these exact commands:
...
When running `devtools::test()`, immediately extract and analyze failures:

1. **Look for failure markers**: `FAIL`, `Error`, `Failure`
2. **Extract context**: Include the test file name, line number, and failure message
3. **Identify patterns**: Group similar failures together
4. **Report concisely**: Summarize unique failure types without repeating full output
...
Extract as:
- **File**: test-shared-instruments.R:36
- **Test**: resolve_instrument_config merges with config
- **Issue**: `result$regex` expected `"^([a-z]+)"` but got `NULL`

## Command Auto-Approval

These commands are pre-approved in `.vscode/settings.json` and do not require user confirmation.
