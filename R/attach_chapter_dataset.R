attach_chapter_dataset <- function(chapter_structure_chapter,
  ...
  data_chapter <- data[, names(data) %in% unique(c(
    dep_vars,
    indep_vars,
    auxiliary_variables
  )), drop = FALSE]
  ...
  filename_chapter_dataset <-
    stringi::stri_c(data_filename_prefix, filename_prefix, chapter_foldername_clean, ".", serialized_format, ignore_null = TRUE)
