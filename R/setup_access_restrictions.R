...
#' @param rel_path_base_to_parent_of_user_restricted_folder String, relative path
#'        from basepath to the folder where the restricted folders are located. (E.g. the "mesos"-folder)
#' @param warn Flag. Whether to provide warning or error if paths do not exist.
#' @param local_main_password_path String. Path to main file containing all
#'        usernames and passwords formatted with a colon between username and password.
#' @param username_folder_matching_df Data frame. If NULL (default), will use
#'        folder names as usernames. Otherwise, a data frame with two columns:
#'        "folder" and "username" where "folder" is the name of the folder and
#'        "username" is the username for that folder.
#' @param universal_usernames Character vector. Usernames in local_main_htpasswd_path
#'        which always have access to folder
#' @param log_rounds Integer, number of rounds in the bcrypt algorithm. The higher the
#'        more time consuming and harder to brute-force.
#' @param append_users Boolean, if TRUE (default) will create new users and add them
#'        to local_main_password_path. See also password_input.
#' @param password_input String, either "prompt" which asks the user for input.
#'        Alternatively, a number stored as string for a generated random password of
#'        said length: "8", "10", "12", "16"
#' @param type Character vector. "netlify" will create _headers file used for Netlify.
#'        "apache" will create .htaccess and .htpasswd files used for general FTP-servers.
#' @param create_main_htaccess Logical. If TRUE, creates a main .htaccess file in
#'        local_basepath with security headers (HSTS, X-Content-Type-Options, X-Frame-Options, etc.).
#'        Only applicable when type includes "apache". Default is FALSE.
#'
...
setup_access_restrictions <- function(remote_basepath = "/home/",
  ...
  if (any("netlify" == type)) {
    create__headers_file(
      remote_basepath = remote_basepath, # Not used in this function, included for consistency
      local_basepath = local_basepath,
      rel_path_base_to_parent_of_user_restricted_folder = rel_path_base_to_parent_of_user_restricted_folder,
