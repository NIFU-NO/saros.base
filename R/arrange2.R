...
arrange2 <- function(data, arrange_vars = NULL, na_first = FALSE) {
  ...
  arrange_exprs <- lapply(names(arrange_vars), function(var) {
    if (is.factor(data[[var]])) {
      if (na_first) {
        expr <- rlang::expr(forcats::fct_relevel(forcats::fct_na_value_to_level(!!rlang::sym(var)), NA))
      } else {
        expr <- rlang::expr(as.integer(!!rlang::sym(var)))
      }
    } else {
      if (na_first) {
        expr <- rlang::expr(dplyr::if_else(is.na(!!rlang::sym(var)), -Inf, !!rlang::sym(var)))
      } else {
        expr <- rlang::expr(!!rlang::sym(var))
      }
    }
    if (arrange_vars[[var]]) {
